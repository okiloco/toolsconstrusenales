...
public class PeticionesResource {
    ...
    public String usuarios(
            ...
            @FormParam("limit") int limit) throws SQLException, ParseException {

        try {
            HashMap<String, Object> hm = dao.listarUsuario(organismo, filtro, start, limit);
            HashMap<String, Object> hmt = dao.listarUsuario(organismo, filtro, 0, 1000000000);

            List list = (List) hm.get("lista");
            List listt = (List) hmt.get("lista");
            return new JSONSerializer()
                    .include(HttpResponse.DATA)
                    .serialize(HttpResponse.message("OK", list, listt.size()));
        } catch (IOException e) {
            return new GenericJSONResponse(false, GenericJSONResponse.GENERAL_BUSINESS_LOGIC_ERROR, "error " + e.getMessage()).toString();
        }
    ...
    public String crearUsuarioPeticion(
            ...
            @FormParam("CHECKED") int checked) throws SQLException, ParseException {

        try {
            if (checked != 0) {

                if (idrol == 0) {
                    return new GenericJSONResponse(false, GenericJSONResponse.GENERAL_BUSINESS_LOGIC_ERROR, "Debe seleccionar un rol").toString();
                }
            }

            GenericJSONResponse mensaje = dao.crearUsuarioPeticiones(organismo, idusuario, nombre, usuarioRegistro, idusuarioDevolucion, idusuarioAprobacion, checked, idrol);
    ...
    public String listarPeticiones(
            ...
            @FormParam("fechradicado") String fechradicad,
            @FormParam("fase") int estadopeticion,
            @FormParam("start") int start,
        ...
        try {
            ...
            HashMap<String, Object> hm = dao.listarPeticiones(organismo, numeroRadicado, usuario, fechradicados, estadopeticion, start, limit);
            HashMap<String, Object> hmt = dao.listarPeticiones(organismo, numeroRadicado, usuario, fechradicados, estadopeticion, 0, 1000000000);
            List list = (List) hm.get("lista");
            List listt = (List) hmt.get("lista");
            return new JSONSerializer()
                    .include(HttpResponse.DATA)
    ...
    public String listarPeticionesUsuario(
        ...
        Date fechradicados = GenericUtils.getDateFromString("yyyy-MM-dd", fechradicad);
        try {
            HashMap<String, Object> hm = dao.listarPeticionesUsuario(organismo, numeroRadicado, usuario, fechradicados, start, limit);
            List list = (List) hm.get("lista");
            return new JSONSerializer()
                    .include(HttpResponse.DATA)
                    .transform(new DateTransformer(DateUtil.FORMAT_SQL), "data.FECREGRADICADO", "data.FECVENCIMIENTO")
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("roles")
    public String roles(@FormParam("organismo") int organismo) throws SQLException, ParseException {

        try {
            HashMap<String, Object> hm = dao.listarRoles(organismo);
            List list = (List) hm.get("lista");
            return new JSONSerializer()
                    .include(HttpResponse.DATA)
                    .serialize(HttpResponse.message("OK", list, list.size()));
    ...
    @POST
    @Produces(MediaType.APPLICATION_JSON)
    @Path("cambiarObservacion")
    public String cambiarobservacion(
            @FormParam("ORGANISMO") int organismo,
            @FormParam("IDREGISTRO_ESTADOS_PETICIONES") int idcambio,
            @FormParam("OBSERVACION") String observacion) throws SQLException, ParseException {

        try {

            GenericJSONResponse mensaje = dao.cambiarObservacion(organismo, idcambio, observacion);
            return mensaje.toString();
